...
1. Pull the codex-docker image:
   ...
   ```bash
   docker pull ghcr.io/icoretech/codex-docker:0.153.3
   ```
...
2. Run the device auth flow (the `codex-bootstrap` helper forces file-based credential storage, which is required for CI):
   ...
   docker run --rm -it \
     ...
     ghcr.io/icoretech/codex-docker:0.153.3 \
...
| `image_version` | No | `0.153.3` | codex-docker image version tag used for the container. |
