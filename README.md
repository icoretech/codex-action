...
1. Pull the codex-docker image:
   ...
   ```bash
   docker pull ghcr.io/icoretech/codex-docker:0.139.0
   ```
...
2. Run the device auth flow (the `codex-bootstrap` helper forces file-based credential storage, which is required for CI):
   ...
   docker run --rm -it \
     ...
     ghcr.io/icoretech/codex-docker:0.139.0 \
...
| `image_version` | No | `0.139.0` | codex-docker image version tag used for the container. |
